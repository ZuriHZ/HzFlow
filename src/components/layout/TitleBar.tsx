...
import { GlobalUpdateBadge, UpdateDetailPanel } from "@/features/updater/components";
        ...
        <>
            <header id="title-bar" className="drag bg-titlebar fixed top-0 right-0 left-0 z-100 flex h-10 items-center justify-between border-b border-sidebar-border px-4 select-none">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold tracking-wide text-white/90">HzFlow</span>
                </div>

                <div className="no-drag flex items-center gap-1.5">
                    <GlobalUpdateBadge onClick={() => setShowPanel(true)} />
            ...
            {showPanel && <UpdateDetailPanel onClose={() => setShowPanel(false)} />}
        </>
    );
}
